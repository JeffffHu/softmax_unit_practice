  ...
  // in: 8'b00(-1.0), 8'bff(0.992), especially, 8'b80(0.0)
...
// Read result
// cmOut -- 定点数Q0.8, 而sum和cmInExp -- 定点数Q16.16, 需要把cmInExp/sum的结果左移8位才能得到Q0.8格式的结果
// 但是, 由于verilog默认mem中的数据和sum都是无符号数, 因此直接相除结果必然是0(做整数除法), 所以正好把sum右移8位变小, 达到相同的效果
// 这个32个32位除法并行运算的资源消耗太大了, 实际应用起来要考虑用其他方法优化
always @(posedge clk)
begin : RESULT_READ
